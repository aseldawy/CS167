...
### III. Run in Distributed Mode (30 minutes)
...
2. On your CS167 machine, download and extract Spark to your `$HOME/cs167` using the command below.
    ```shell
    curl https://dlcdn.apache.org/spark/spark-3.5.4/spark-3.5.4-bin-without-hadoop.tgz | tar -xvz -C $HOME/cs167
    ```
3. Set the environment variables `$SPARK_HOME` and `$PATH` as follows:
    ```shell
    cp ~/.bashrc ~/.profile
...
4. Configure Spark to use Hadoop classes that we installed in Lab 3.
...
9. Run your program in your CS167 machine using the following command. Do not forget to replace `[UCRNetID]` with the correct one. 
```shell
spark-submit --class edu.ucr.cs.cs167.[UCRNetID].App [UCRNetID]_lab5-1.0-SNAPSHOT.jar nasa_19950801.tsv
```

Note that, you need to have the input file also in your CS167 machine home directory and also put that file into HDFS before running the `spark-submit` command.
...
10. To use the cluster that we just started, change the following code from
...
8. Run your program again in your CS167 machine with the following parameters `nasa_19950801.tsv filter_output 200`.
    ...
    ```bash
    spark-submit --class edu.ucr.cs.cs167.[UCRNetID].Filter [UCRNetID]_lab5-1.0-SNAPSHOT.jar nasa_19950801.tsv filter_output_[UCRNetID] 200
    ```
